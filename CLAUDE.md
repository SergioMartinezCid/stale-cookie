...
- **Popup panels are height-capped (~600 px) by both browsers**: a document taller than the cap grows a second (near-useless) outer scrollbar. The popup body is a flex column with `max-height: 560px` and only `#results` scrolls — keep it that way when adding popup UI.
