...
- **Preview**: cleaning always shows what will be deleted before committing (at minimum in manual mode). One row per site (eTLD+1 of the visit domain); all data types share the row and delete together. The preview (outcome + checkbox overrides) survives popup teardown via a `storage.session` cache (`SCAN_CACHE_TTL_MS` 15 min, never on disk — visit-memory rule); invalidated by any deletion (cleared at the START of `deleteGroups`), undo restore, or settings write, and dismissible with the popup's "Clear results" button. Partitioned (CHIPS) cookies fold into their **partition top-level site's** row — staleness is judged by visits to that site, not the cookie's own domain. Container cookie groups fold into their domain's row (badges shown).
...
- **Deletion executes in the background**: the popup sends a `delete-groups` runtime message (`DeleteGroupsRequest` in `ext/scanner.ts`) — popup pages are torn down on any outside click, which must never leave a deletion half-done and unlogged. The background handler does the bookkeeping (action log, reminder reset). Scan/preview stay in the popup (read-only; the outcome is cached in `storage.session` — see product rules). `deleteGroups` is resilient (per-item/per-group error isolation, partial counts always reach the action log; `runAutoClean` reschedules its next alarm in a `finally`) and deletes through a bounded worker pool (`DELETE_CONCURRENCY` 10) — sequential awaits made thousand-item deletions crawl; the popup shows "Deleting…" meanwhile.
...
- **Popup: only `#results` may scroll; the document must never.** The body is a flex column (`max-height: 560px` is just a ceiling — the real panel cap varies with screen/window/OS display scaling, so `fitPopupHeight()` clamps the body to the measured viewport), and `html` has `overflow: hidden`: Firefox's panel keeps a phantom scroll range from its intrinsic-size pass when the document holds a large scroll container (an outer scrollbar that "scrolls into emptiness"). That phantom is reproducible ONLY in a real toolbar popup, never in a tab — integration tests cannot catch it; keep this layout when adding popup UI.
