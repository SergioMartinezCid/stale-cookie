...
- **Manifest V3**, cross-browser WebExtensions common subset, **no MV2 split**: one shared JS bundle; only the manifest differs per target, generated at build time — `dist/` (Firefox, event page, strict_min_version 142) and `dist-chrome/` (service worker, `minimum_chrome_version` 123, `browser_specific_settings` dropped, `contextualIdentities` stripped). **Standing instruction: flag MV3 limitations to the user as they are encountered.**
