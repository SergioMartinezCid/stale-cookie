...
- **Firefox extras.** Container cookies and partitioned (CHIPS) cookies are handled and labeled in the preview.
- Dark and light themes. English and Spanish.

**Privacy**

No network requests, ever. No telemetry, no analytics. Everything the extension needs is stored locally in your browser and never leaves it. The extension runs no content scripts — it never reads or touches web pages. Full policy: [PRIVACY.md](https://github.com/SergioMartinezCid/stale-cookie/blob/main/PRIVACY.md)

**Known limitations**

- Firefox only exposes the current session's downloads to extensions, so older download entries can't be listed or deleted per site.
...
- Desktop only; sites are matched by registrable domain (visiting mail.google.com keeps google.com data fresh).

Open source (MIT): [github.com/SergioMartinezCid/stale-cookie](https://github.com/SergioMartinezCid/stale-cookie)
...
- **Extras de Firefox.** Las cookies de contenedores y las cookies particionadas (CHIPS) se gestionan y etiquetan en la vista previa.
- Temas claro y oscuro. Español e inglés.

**Privacidad**

Sin conexiones de red, nunca. Sin telemetría ni analítica. Todo lo que la extensión necesita se guarda localmente en tu navegador y nunca sale de él. La extensión no ejecuta scripts de contenido: nunca lee ni toca las páginas web. Política completa: [PRIVACY.md](https://github.com/SergioMartinezCid/stale-cookie/blob/main/PRIVACY.md)

**Limitaciones conocidas**

- Firefox solo expone a las extensiones las descargas de la sesión actual, así que las entradas de descargas antiguas no pueden listarse ni eliminarse por sitio.
...
- Solo escritorio; los sitios se emparejan por dominio registrable (visitar mail.google.com mantiene frescos los datos de google.com).

Código abierto (MIT): [github.com/SergioMartinezCid/stale-cookie](https://github.com/SergioMartinezCid/stale-cookie)
